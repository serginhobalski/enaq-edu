...
class ItqController extends Controller
{
    ...
    public function index()
    {
        ...
        $cursos = Curso::all()->where('categoria', '=', 'enaq');
        $usuario = Auth::user();
        $data = [
            'titulo' => 'Painel',
            'subtitulo' => 'Plataforma de Cursos do Postulante',
            'cursos' => $cursos,
            'usuario' => $usuario,
    ...
    public function meus_avisos()
    {
        $avisos = Aviso::all();

        $data = [
            'titulo' => 'Meus Avisos',
            'subtitulo' => 'Avisos de ' . Auth::user()->name,
            'avisos' => $avisos,
        ];
        return view('itq.avisos', $data);
    }
