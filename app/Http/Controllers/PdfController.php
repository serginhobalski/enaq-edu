        ...
        if (request()->ajax()) {
            ...
            foreach ($pdfs as $pdf) {
                $data[] = [
                    ...
                    'acoes' => '<a href="' . route('pdfs.show', $pdf->id) . '" title="Visualizar PDF"><i class="fa-solid fa-eye text-info"></i></a> <a href="' . route('pdfs.edit', $pdf->id) . '" title="Editar PDF"><i class="fa-solid fa-pen-to-square text-success"></i></a> <a href="' . route("pdfs.destroy", $pdf->id) . '" title="Deletar PDF" onclick="event.preventDefault(); document.getElementById(\'delete-form-' . $pdf->id . '\').submit();"><i class="fa-solid fa-trash text-danger"></i></a> <form id="delete-form-' . $pdf->id . '" method="post" action="' . route("pdfs.destroy", $pdf->id) . '">' . csrf_field() . method_field("DELETE") . '</form>',
                ];
            }
            $retorno = [
                'data' => $data,
